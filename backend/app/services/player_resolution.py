...
class PlayerResolutionService:
    ...
    def resolve_player_from_text(db: Session, text: str) -> Tuple[Optional[Player], str, List[Dict[str, Any]]]:
        ...
        exact_match = db.query(Player).filter(
            func.lower(Player.displayName) == text.lower()
        ).first()
        if exact_match:
            return exact_match, 'exact', []
        
        # 2. Try parsing "Lastname, Firstname" format
        if ',' in text:
        ...
        # 3. Try parsing "Firstname Lastname" format
        ...
        # 4. Partial match on displayName (contains)
        ...
        # 5. Try first name + last name partial matching
        if len(name_parts) >= 2:
            first_name = name_parts[0]
            last_name = ' '.join(name_parts[1:])
        ...
        # 6. Try last name only (if single word)
        if len(name_parts) == 1:
            last_name_matches = db.query(Player).filter(
                Player.lastName.ilike(f"%{text}%")
            ).all()
        ...
        # 7. Try shortName if available
        short_name_matches = db.query(Player).filter(
            Player.shortName.ilike(f"%{text}%")
        ).all()
