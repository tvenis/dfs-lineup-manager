#!/usr/bin/env python3
"""
Test script to verify the corrected field mapping.
Tests that RST% maps to ownership_percentage and FPTS maps to projected_points.
"""
...
def test_corrected_mapping():
    """Test the corrected field mapping with specific validation."""
    print("🚀 Corrected Field Mapping Test")
    print("="*60)
                ...
                if players:
                    ...
                    for player in players:
                        if "Christian McCaffrey" in player.get('name', ''):
                            ...
                            print(f"   Match: {'✅' if abs(actual_ownership - expected_ownership) < 0.1 else '❌'}")
                            
                            print(f"   Expected projected (FPTS): {expected_projected}")
                            print(f"   Actual projected: {actual_projected}")
                            print(f"   Match: {'✅' if abs(actual_projected - expected_projected) < 0.1 else '❌'}")
                    ...
                    print(f"\n🔍 FIRST 5 PLAYERS (Field Validation):")
                    print("-" * 60)
...
def main():
    """Run the corrected mapping test."""
    success = test_corrected_mapping()
    
    if success:
        print("\n✅ Test completed successfully!")
        print("\n💡 The corrected scrape_ownership_data function should now:")
